			...
			try {
				...
				System.out.println(
					"-->Servidor da plataforma "
						+ AgentPlataformDescription.getInstance().getName()
						+ " recebe conexao de um cliente<--");
	...
	public class AccServer extends Thread {
		...
		*/
		public void run() {
			...
			while (num < descriptions.size()) {
				envs.add(
					((EnvironmentDescription) descriptions.get(num))
						.getEnvironment());
				num++;
			}
		...
		*/
		private ArrayList sendMessage(ArrayList envs, Message message) {
			...
			AgentPlataformDescription dsc =
				AgentPlataformDescription.getInstance();
			...
			while (control < qtdAgentsReceivers) {
				...
				while (num < receivers.size()) {
					if (id
						.getName()
						.compareToIgnoreCase((String) receivers.get(num))
						== 0) {
						value = true;
						break;
					}
				...
				if (!value) {
					...
					aux.setComments(
						((ElementID) listAux.get(0)).getName()
							+ " tentou enviar a mensagem para "
							+ ((ElementID) to.get(control)).getName()
							+ " mas não conseguiu, pois este não existe na plataforma "
							+ plataformName
							+ ".");
				} else {
			...
			while (control < noReceivers.size()) {
				...
				aux.setComments(
					((ElementID) listAux.get(0)).getName()
						+ " tentou enviar a mensagem para "
						+ ((ElementID) to.get(control)).getName()
						+ " mas este não pertence a plataforma "
						+ plataformName
						+ ".");
				messages.add(aux);
				control++;
